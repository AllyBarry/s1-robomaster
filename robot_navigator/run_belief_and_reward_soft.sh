#!/bin/bash
# Soft-hold variant of ./run_belief_and_reward.sh — identical bundle
# (global_feedback + per-robot beliefs + trajectory logger + optional
# video recorder) but with continuous, peer-kinematics-driven
# importance weighting enabled on every belief node.
#
# Each per-robot belief weights its BLR samples by how attributable Δr
# is to *its own* motion vs concurrent peer motion (observed via the
# existing /field/robot_{i}/pose subscriptions). Single-mover ticks
# update the posterior at full strength; concurrent-motion ticks are
# damped automatically. Zero additional bandwidth — the peer poses are
# already on the wire for repulsion / mask. Composes orthogonally with
# the 1-bit hold (run_belief_and_reward_hold.sh): you can flip both on
# by editing the launch line.
#
# Comparison ladder:
#   ./run_belief_and_reward.sh        # 0-bit baseline (no coordination)
#   ./run_belief_and_reward_hold.sh   # 1-bit hard hold (peers freeze on bit)
#   ./run_belief_and_reward_soft.sh   # 1-bit soft (importance weighting)
#
# Output run folders are timestamp-prefixed so all three variants live
# side by side under ./experiment_logs/.
#
# Usage:
#   ./run_belief_and_reward_soft.sh [flags]
#
...
# Distinct scenario/run-folder stem so soft runs don't overwrite or
# visually merge with baseline / hold runs. Explicit --run-name still wins.
SCENARIO_NAME="${SCENARIO:-run_soft}"
if [ -z "${RUN_NAME}" ]; then
...
LAUNCH_ARGS+=("soft_hold_enabled:=true")
...
# Separate container name so a stale baseline / hold container doesn't
# block us (and vice-versa). Still a singleton — can't coexist with a
# live baseline or hold run because all three fight over /global_reward.
CONTAINER_NAME="belief_and_reward_soft"
...
echo "Started ${CONTAINER_NAME} (SOFT model) — follow with: docker logs -f ${CONTAINER_NAME}"
echo ""
